    ...
    <div className="space-y-8">
      ...
      <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
        <div className="flex items-center gap-4">
          <div className="p-3 bg-gray-50 text-black rounded-2xl border border-gray-200">
      ...
      <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
        ...
        <form onSubmit={handleUpload} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            <label className="block text-sm font-bold text-black">Banner Image</label>
            <div
          ...
          <div className="space-y-4 flex flex-col justify-between">
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-bold text-black mb-1.5">
                  Title / Caption <span className="text-gray-500 font-normal">(Optional)</span>
                </label>
                <input
                  type="text"
                  value={title}
                  onChange={(e) => setTitle(e.target.value)}
                  className="w-full px-4 py-3 bg-white border border-gray-300 rounded-xl text-sm text-black placeholder-gray-400 outline-none focus:border-black focus:ring-1 focus:ring-black transition"
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-white border border-gray-300 rounded-xl text-sm text-black outline-none focus:border-black focus:ring-1 focus:ring-black transition"
                />
              </div>
              ...
              {uploadSuccess && (
                <div className="bg-green-50 text-green-700 border border-green-200 rounded-xl p-3 text-sm font-medium">
                  {uploadSuccess}
                </div>
              )}
            </div>
            ...
            <button
              ...
              className="w-full py-3.5 bg-black text-white font-bold rounded-xl flex justify-center items-center gap-2 hover:bg-gray-800 transition duration-200 cursor-pointer shadow-md disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none"
            >
              {uploading ? (
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
        ...
        {loading ? (
          ...
          <div className="text-center py-12 text-gray-500">
            ...
            <p className="font-semibold text-black">No carousel images found.</p>
            <p className="text-xs text-gray-500 mt-1">Upload your first image above.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {carousels.map((item) => (
              <div
                key={item._id}
                className="bg-white border border-gray-200 rounded-2xl overflow-hidden group hover:border-black transition duration-200 flex flex-col justify-between shadow-sm hover:shadow-md"
              >
                <div>
                  <div className="h-44 w-full relative overflow-hidden bg-gray-50">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center">
                      <button
                        ...
                        className="p-3 bg-white text-red-600 rounded-full hover:bg-red-50 hover:scale-105 transition-all cursor-pointer shadow-sm"
                        title="Delete Image"
                      >
                ...
                <div className="px-4 pb-4 pt-2 border-t border-gray-100 flex items-center justify-between">
                  <span className="text-xs font-medium text-gray-500">
                    Order: <strong className="text-black font-bold">{item.order}</strong>
                  </span>

                  <button
                    onClick={() => toggleStatus(item._id, item.isActive)}
                    className={`text-xs px-4 py-1.5 rounded-full font-bold transition-all cursor-pointer border ${
                      item.isActive
                        ? 'bg-black text-white border-black hover:bg-gray-800 hover:border-gray-800'
                        : 'bg-white text-gray-500 border-gray-300 hover:bg-gray-50 hover:text-black'
                    }`}
                  >
